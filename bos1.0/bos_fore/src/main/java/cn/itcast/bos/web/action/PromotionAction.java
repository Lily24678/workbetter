...
public class PromotionAction extends BaseAction<Promotion> {
	...
	public String pageQuery() {
		...
		PageBean<Promotion> pageBean = WebClient
				.create("http://localhost:9001/bos_management/services/promotionService/pageBean?page="
						+ page + "&rows=" + rows)
			...
			if (!htmlFile.exists()) {
				...
				Promotion promotion = WebClient
						.create(Constant.BOS_MANAGEMENT_URL
								+ "/services/promotionService/promotion/findById/"
								+ model.getId())
						.accept(MediaType.APPLICATION_JSON)
						.get(Promotion.class);
				Map<String, Object> paramsMap = new HashMap<String, Object>();
				paramsMap.put("promotion", promotion);
				// 合并输出
				template.process(paramsMap, new OutputStreamWriter(
